...
`pneumatik-decrypt.py` turns one back into a plain zip. It needs Python 3 and
the `cryptography` package, and nothing else — not Pneumatik, not its database,
not a checkout:

```sh
curl -O https://raw.githubusercontent.com/Klackwerk/Pneumatik-Database-Backups/main/tools/pneumatik-decrypt.py
pip install cryptography
python3 pneumatik-decrypt.py -k pneumatik.key orders_prod_20260801_0230.sql.zip.enc
# → orders_prod_20260801_0230.sql.zip
```

Keep a copy of that script next to your keyfile. It is 150 lines and the format
it reads is documented in its header, so a restore never waits on GitHub either.

Straight into a database, without the plaintext ever hitting disk:

```sh
python3 pneumatik-decrypt.py -k pneumatik.key -o - orders.sql.zip.enc \
