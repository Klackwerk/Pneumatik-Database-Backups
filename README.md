...
Requirements: Docker with Compose. No clone needed — two files and a key:
...
Edit `.env` and set `DB_PASSWORD`, `DB_ROOT_PASSWORD` and
`PNEUMATIK_JWT_SECRET` (`openssl rand -base64 48`), then:
...
```
ghcr.io/klackwerk/pneumatik-database-backups:3.2.1   # exact release
ghcr.io/klackwerk/pneumatik-database-backups:3.2     # newest 3.2.x (compose default)
ghcr.io/klackwerk/pneumatik-database-backups:latest  # newest release
ghcr.io/klackwerk/pneumatik-database-backups:edge    # newest main, untagged
```

Pin a different one with `PNEUMATIK_VERSION` in `.env`. `linux/amd64` only.
