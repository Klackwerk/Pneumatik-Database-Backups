...
function ActivityTooltip({
  ...
  return (
    <div className="border rounded bg-body px-3 py-2 shadow-sm small">
      <p className="mb-1 fw-medium">{formatDay(label)}</p>
      <p className="mb-0 d-flex align-items-center gap-2">
        <span className="d-inline-block rounded-1" style={{ width: 8, height: 8, background: 'var(--chart-good)' }} aria-hidden />
        Finished: <span className="fw-medium">{finished}</span>
      </p>
      <p className="mb-0 d-flex align-items-center gap-2">
        <span className="d-inline-block rounded-1" style={{ width: 8, height: 8, ...failedSwatch }} aria-hidden />
        Failed: <span className="fw-medium">{failed}</span>
      </p>
    </div>
  )
}
        ...
        <BarChart data={activity} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
          <defs>
            ...
            >
              ...
              <line x1={0} y1={0} x2={0} y2={6} stroke="var(--bs-body-bg)" strokeWidth={2.5} opacity={0.65} />
            </pattern>
          </defs>
          <CartesianGrid vertical={false} stroke="var(--bs-border-color)" />
          <XAxis
            dataKey="date"
          ...
          />
          <YAxis allowDecimals={false} axisLine={false} tickLine={false} tick={{ fill: 'var(--bs-secondary-color)', fontSize: 11 }} />
          <Tooltip content={<ActivityTooltip />} cursor={{ fill: 'var(--bs-tertiary-bg)', opacity: 0.5 }} />
          <Bar dataKey="finished" name="Finished" stackId="day" fill="var(--chart-good)" stroke="var(--bs-body-bg)" strokeWidth={1} isAnimationActive={false} />
          <Bar
            dataKey="failed"
            name="Failed"
            stackId="day"
